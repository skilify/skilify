        ...
        {value &&
          value.map((tag) => (
            ...
            >
              ...
              <span className="text-sm font-sans whitespace-nowrap">{tag}</span>
            </div>
          ))}
      </div>
      <input
    ...
    fetch(`/api/profile/me`, {
      ...
      .then((res) => {
        if (res.error) {
          ...
          mutate({ user: { ...data?.user, ...submitData } } as any, false);
          setEditing(false);
        }
      })
            ...
            <div className="flex flex-col items-start justify-between space-y-2 w-72">
              ...
              <div className="flex flex-row gap-4">
                <h2 className="text-3xl font-bold tracking-tight">
                  {data ? data.user.name : <Skeleton className="h-9 w-32" />}
                </h2>
                {data ? (
                  <HoverCard>
              ...
              >
                {editing ? (
                  ...
                  <>
                    ...
                    {session && data && session.user.id === data.user.id && (
